function Set_Partition(dataset, num_gal, num_test, fixprobcam, profix)
load(['dataset/' dataset '_Images' profix '.mat']);
...
%             warning('Needs more parameters for unknown dataset');
                        % use half of the dataset as training
            num_test = floor(numel(unique(gID))/2);
    end
end
if num_gal > 1
...
if fixprobcam % prob image from the same camera
    uCid = unique(camID);    
    for idx_partition = 1:num_partition        
        for c = 1:numel(uCid)
            ID_train = gID(Partition(idx_partition).idx_train);
            ID_test = gID(Partition(idx_partition).idx_test);
            idx_train_gallery = ones(1, length(Partition(idx_partition).idx_train));
            idx_test_gallery = ones(1, length(Partition(idx_partition).idx_test));
            
            idx_train_gallery(camID(Partition(idx_partition).idx_train) == c) = 0;
            [ix_pos_pair, ix_neg_pair]=GeneratePair(ID_train,npratio);
            Partition(idx_partition).ix_train_gallery(c,:) = idx_train_gallery==1;
            Partition(idx_partition).idx_train_pos_pair = uint16(ix_pos_pair);
            Partition(idx_partition).idx_train_neg_pair = uint16(ix_neg_pair);
            
            [ix_pos_pair, ix_neg_pair]=GeneratePair(ID_test,npratio);
            idx_test_gallery(camID(Partition(idx_partition).idx_test) == c) = 0;
            Partition(idx_partition).ix_test_gallery(c,:) = idx_test_gallery==1;
            Partition(idx_partition).idx_test_pos_pair = uint16(ix_pos_pair);
            Partition(idx_partition).idx_test_neg_pair = uint16(ix_neg_pair);
        end
    end
    save(['Feature/' dataset '_Partition_' partition_name profix '.mat'], 'Partition', '-v7.3');
    return;
end

% generate gallery part and prob part for both training and testing data.
for idx_partition=1:10%num_partition
    idx_partition
    ID_train = gID(Partition(idx_partition).idx_train);
    ...
    for m =1: length(unique(ID_train)) % for each person random choosing gallery sample
        ...
        iix_temp = zeros(num_gal,num_trail);
        for t = 1:num_trail
